...
> The Alpine list is the maintained one -- it is what CI installs. The Debian
> list is a much shorter legacy file and is not enough on its own; see
> [Debian](#debian) below.
...
mender-testkit requires Python 3.10 or newer. CI runs the suite on the Python
that ships in the `docker:28-dind` image, currently 3.12.
...
`deb-requirements.txt` has not kept pace with the Alpine list -- it still names
`docker-compose` (the retired v1 tool; everything here uses the `docker compose`
v2 plugin) and `python-pip`, and it is missing packages the tests shell out to,
among them `openssh-client`, `jq`, `e2fsprogs-extra` (for `debugfs`), `zstd`,
`skopeo` and `iptables`. Treat the Alpine list as the source of truth for what
is actually needed and install the Debian equivalents by hand.
...
> The `:pn-` override syntax above is for Yocto 3.4 (honister) and newer, which
> covers the scarthgap meta-mender this repository tracks. On older releases the
> separator is an underscore: `PREFERRED_VERSION_pn-mender`.
