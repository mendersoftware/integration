...
if [ $# -eq 0 ]; then
    py.test $XDIST_ARGS $MAX_FAIL_ARG -s --verbose --junitxml=results.xml $HTML_REPORT --runfast --runslow $UPGRADE_TEST_ARG tests/test_basic_integration.py::TestBasicIntegration::test_failed_updated_and_valid_update
    exit $?
fi
