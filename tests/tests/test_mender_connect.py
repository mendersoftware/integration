...
class _TestRemoteTerminalBase:
    ...
    def test_in_poor_network_environment(self, docker_env):
        ...
        mender_client_ip=docker_env._docker_compose_cmd("ps mender-client --format '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}'")
        api_gateway_ip=docker_env._docker_compose_cmd("ps mender-api-gateway --format '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}'")
