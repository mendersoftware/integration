...
class TestDeltaDockerCompose(MenderTesting):
    ...
    def test_delta_first_delta_rollback_preserves_source(
        ...
        rollback target is a prior committed delta (A); here the store has no prior
        *delta*, only the source composition installed via the base module.
        A failed update must be a no-op for the device as a whole, so after rollback
        the source composition must still be running and the failed target must
